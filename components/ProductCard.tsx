  ...
  return (
    <article className="product-card">
      <div className="product-image-wrap">
        <Image src={product.image} alt={name} fill sizes="(max-width: 700px) 50vw, (max-width: 1050px) 33vw, 260px" className="cover" />
      ...
      <div className="product-meta">
        ...
        <h3>{name}</h3>
        <strong>¥{product.price.toLocaleString()} <em>（{labels.tax}）</em></strong>
      </div>
    </article>
  );
}
